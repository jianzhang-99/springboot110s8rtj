...
<div :style='{"width":"100%","padding":"20px 10%","margin":"30px auto","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="种植时间" prop="zhongzhishijian">
            <el-input v-model="ruleForm.zhongzhishijian" 
                placeholder="种植时间" clearable :disabled=" false  ||ro.zhongzhishijian"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="评估图片" v-if="type!='cross' || (type=='cross' && !ro.pinggutupian)" prop="pinggutupian">
            <file-upload
            tip="点击上传评估图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' class="upload" v-else label="评估图片" prop="pinggutupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.pinggutupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="气候条件" prop="qihoutiaojian">
            <el-input v-model="ruleForm.qihoutiaojian" 
                placeholder="气候条件" clearable :disabled=" false  ||ro.qihoutiaojian"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}'  label="土壤类型" prop="turangleixing">
            <el-select v-model="ruleForm.turangleixing" placeholder="请选择土壤类型" :disabled=" false  ||ro.turangleixing" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="产量表现" prop="chanliangbiaoxian">
            <el-input v-model="ruleForm.chanliangbiaoxian" 
                placeholder="产量表现" clearable :disabled=" false  ||ro.chanliangbiaoxian"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="评估时间" prop="pinggushijian">
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="适应性评估" prop="shiyingxingpinggu">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="适应性评估"
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='pinggutupian'){
              this.ruleForm.pinggutupian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zuowushiyingxingpinggu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zuowushiyingxingpinggu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zuowushiyingxingpinggu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      pinggutupianUploadChange(fileUrls) {
          this.ruleForm.pinggutupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
