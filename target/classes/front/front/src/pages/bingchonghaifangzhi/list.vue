...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('bingchonghaifangzhi','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/bingchonghaifangzhiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"30px 0 0","flexWrap":"wrap","background":"#ecf2ea","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid red","width":"200px","padding":"0px","overflow":"hidden","height":"200px"}' class="img-box">
				<img @click.stop="imgPreView(item.bingchongtupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.bingchongtupian && item.bingchongtupian.substr(0,4)=='http'" :src="item.bingchongtupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.bingchongtupian?item.bingchongtupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.bingchongtupian?item.bingchongtupian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				...
				<div class="title">种植面积(㎡):{{item.zhongzhimianji}}</div>
				<div class="title">{{item.bingchonghaizhonglei}}</div>
		        ...
		        <div :style='{"padding":"0 10px","borderColor":"red","borderStyle":"solid","borderWidth":"0 0 0px 0","display":"inline-block"}' class="publisher">
		          ...
		          <span class="text">{{item.zhongzhihuzhanghao}}</span>
		        </div>
		      </div>
		      <div class="desc">{{item.fangzhicuoshi}}</div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'病虫害种类',list:this.bingchonghaizhongleiOptions,tableName: 'bingchonghaizhonglei',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.bingchonghaizhonglei != '') searchWhere.bingchonghaizhonglei = this.formSearch.bingchonghaizhonglei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`bingchonghaifangzhi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/bingchonghaifangzhiDetail', query: params});
      },
