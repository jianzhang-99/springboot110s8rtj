...
<div :style='{"width":"100%","padding":"20px 10%","margin":"30px auto","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="检测项目" prop="jiancexiangmu">
            <el-input v-model="ruleForm.jiancexiangmu" 
                placeholder="检测项目" clearable :disabled=" false  ||ro.jiancexiangmu"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}'  label="检测结果" prop="jiancejieguo">
            <el-select v-model="ruleForm.jiancejieguo" placeholder="请选择检测结果" :disabled=" false  ||ro.jiancejieguo" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="溯源码" v-if="type!='cross' || (type=='cross' && !ro.suyuanma)" prop="suyuanma">
            <file-upload
            tip="点击上传溯源码"
            action="file/upload"
            :limit="3"
            :multiple="true"
            :fileUrls="ruleForm.suyuanma?ruleForm.suyuanma:''"
            @change="suyuanmaUploadChange"
            ></file-upload>
          </el-form-item>
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' class="upload" v-else label="溯源码" prop="suyuanma">
                <img v-if="ruleForm.suyuanma.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.suyuanma.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.suyuanma.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="认证机构" prop="renzhengjigou">
            <el-input v-model="ruleForm.renzhengjigou" 
                placeholder="认证机构" clearable :disabled=" false  ||ro.renzhengjigou"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="检测报告" prop="jiancebaogao">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jiancexiangmu'){
              this.ruleForm.jiancexiangmu = obj[o];
              this.ro.jiancexiangmu = true;
              continue;
            }
            if(o=='jiancejieguo'){
            ...
            if(o=='suyuanma'){
              this.ruleForm.suyuanma = obj[o].split(",")[0];
              this.ro.suyuanma = true;
              continue;
            }
            if(o=='renzhengjigou'){
              this.ruleForm.renzhengjigou = obj[o];
              this.ro.renzhengjigou = true;
              continue;
            }
            if(o=='jiancebaogao'){
        ...
        this.jiancejieguoOptions = "正常,异常".split(',')
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zhiliangjiancehesuyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zhiliangjiancehesuyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zhiliangjiancehesuyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      suyuanmaUploadChange(fileUrls) {
          this.ruleForm.suyuanma = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
