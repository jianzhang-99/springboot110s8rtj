		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 20px","borderRadius":"0px","textAlign":"left","background":"none","display":"block","width":"100%","fontSize":"inherit","order":"2"}' >
					...
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="病虫害类型" prop="bingchonghaileixing">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">病虫害类型</label>
						<el-select clearable v-model="searchForm.bingchonghaileixing" placeholder="请选择病虫害类型" >
							<el-option v-for="(item,index) in bingchonghaileixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">推荐农药</label>
						<el-input v-model="searchForm.tuijiannongyao" placeholder="推荐农药" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('bingchonghaicuoshi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('bingchonghaicuoshi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('bingchonghaicuoshi','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="bingchonghaileixing"
						label="病虫害类型">
						<template slot-scope="scope">
							{{scope.row.bingchonghaileixing}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="推荐农药">
						<template slot-scope="scope">
							{{scope.row.tuijiannongyao}}
						</template>
					</el-table-column>
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="fengmiantupian" width="200" label="封面图片">
						<template slot-scope="scope">
							<div v-if="scope.row.fengmiantupian">
								<img v-if="scope.row.fengmiantupian.substring(0,4)=='http'" :src="scope.row.fengmiantupian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.fengmiantupian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('bingchonghaicuoshi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('bingchonghaicuoshi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('bingchonghaicuoshi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.bingchonghaileixing!='' && this.searchForm.bingchonghaileixing!=undefined){
            params['bingchonghaileixing'] = this.searchForm.bingchonghaileixing
          }
           if(this.searchForm.tuijiannongyao!='' && this.searchForm.tuijiannongyao!=undefined){
            params['tuijiannongyao'] = '%' + this.searchForm.tuijiannongyao + '%'
          }
      this.$http({
        url: "bingchonghaicuoshi/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	bingchonghaicuoshistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'bingchonghaicuoshi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
