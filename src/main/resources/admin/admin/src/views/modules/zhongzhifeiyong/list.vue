		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 20px","borderRadius":"0px","textAlign":"left","background":"none","display":"block","width":"100%","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="支出类型" prop="zhichuleixing">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">支出类型</label>
						<el-select clearable v-model="searchForm.zhichuleixing" placeholder="请选择支出类型" >
							<el-option v-for="(item,index) in zhichuleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('zhongzhifeiyong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('zhongzhifeiyong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('zhongzhifeiyong','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="piaoju" width="200" label="票据">
						<template slot-scope="scope">
							<div v-if="scope.row.piaoju">
								<img v-if="scope.row.piaoju.substring(0,4)=='http'" :src="scope.row.piaoju.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.piaoju.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zhongzhifeiyong','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('zhongzhifeiyong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zhongzhifeiyong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
...
import AddOrUpdate from "./add-or-update";
    ...
    init () {
          this.zhichuleixingOptions = "种子,农药,肥料,其它".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.zhichuleixing!='' && this.searchForm.zhichuleixing!=undefined){
            params['zhichuleixing'] = this.searchForm.zhichuleixing
          }
    params['sort'] = 'dengjiriqi';
    params['order'] = 'desc';
      this.$http({
        url: "zhongzhifeiyong/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	zhongzhifeiyongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'zhongzhifeiyong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
