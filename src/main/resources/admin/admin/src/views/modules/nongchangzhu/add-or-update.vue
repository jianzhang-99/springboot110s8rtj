	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="农场主电话" prop="nongchangzhudianhua">
					<el-input v-model="ruleForm.nongchangzhudianhua" placeholder="农场主电话" clearable  :readonly="ro.nongchangzhudianhua"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="农场主电话" prop="nongchangzhudianhua">
					<el-input v-model="ruleForm.nongchangzhudianhua" placeholder="农场主电话" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				nongchangzhuzhanghao: [
					{ required: true, message: '农场主账号不能为空', trigger: 'blur' },
				],
				mima: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='nongchangzhudianhua'){
							this.ruleForm.nongchangzhudianhua = obj[o];
							this.ro.nongchangzhudianhua = true;
							continue;
						}
				}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `nongchangzhu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.nongchangzhuCrossAddOrUpdateFlag = false;
						 this.parent.search();
