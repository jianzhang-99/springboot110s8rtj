	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="个人介绍" prop="gerenjieshao">
					<editor 
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.gerenjieshao" label="个人介绍" prop="gerenjieshao">
                    <span :style='{"padding":"0 20px","color":"inherit","background":"#fff","display":"inline-block","width":"80%","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.gerenjieshao"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				jishuyuanzhanghao: [
					{ required: true, message: '技术员账号不能为空', trigger: 'blur' },
				],
				mima: [
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.gerenjieshao = this.ruleForm.gerenjieshao.replace(reg,'../../../springboot110s8rtj/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `nongyejishuyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.nongyejishuyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
