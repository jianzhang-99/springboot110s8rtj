<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 20px","borderRadius":"0px","textAlign":"left","background":"none","display":"block","width":"100%","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">设备名称</label>
						<el-input v-model="searchForm.shebeimingcheng" placeholder="设备名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">设备分类</label>
						<el-input v-model="searchForm.shebeifenlei" placeholder="设备分类" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('shebeixinxi','设备维护登记')" @click="shebeiweihuqingkuangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<shebeiweihuqingkuang-cross-add-or-update v-if="shebeiweihuqingkuangCrossAddOrUpdateFlag" :parent="this" ref="shebeiweihuqingkuangCrossaddOrUpdate"></shebeiweihuqingkuang-cross-add-or-update>
...
import shebeiweihuqingkuangCrossAddOrUpdate from "../shebeiweihuqingkuang/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				shebeiweihuqingkuangCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			shebeiweihuqingkuangCrossAddOrUpdate,
		},
		methods: {
			...
			shebeiweihuqingkuangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shebeiweihuqingkuangCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shebeiweihuqingkuangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shebeiweihuqingkuangCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	shebeixinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'shebeixinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
