...
<div :style='{"width":"100%","padding":"20px 10%","margin":"30px auto","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}'  label="生长阶段" prop="shengzhangjieduan">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="叶片状况" prop="yepianzhuangkuang">
            <el-input v-model="ruleForm.yepianzhuangkuang" 
                placeholder="叶片状况" clearable :disabled=" false  ||ro.yepianzhuangkuang"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="花果状况" prop="huaguozhuangkuang">
            <el-input v-model="ruleForm.huaguozhuangkuang" 
                placeholder="花果状况" clearable :disabled=" false  ||ro.huaguozhuangkuang"></el-input>
          </el-form-item>
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        ro:{
				...
				yepianzhuangkuang : false,
				huaguozhuangkuang : false,
				dengjiriqi : false,
        },
        type: '',
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='yepianzhuangkuang'){
              this.ruleForm.yepianzhuangkuang = obj[o];
              this.ro.yepianzhuangkuang = true;
              continue;
            }
            if(o=='huaguozhuangkuang'){
              this.ruleForm.huaguozhuangkuang = obj[o];
              this.ro.huaguozhuangkuang = true;
              continue;
            }
            if(o=='dengjiriqi'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`shengzhangqingkuang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('shengzhangqingkuang/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`shengzhangqingkuang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
