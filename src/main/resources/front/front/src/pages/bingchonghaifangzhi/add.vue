...
<div :style='{"width":"100%","padding":"20px 10%","margin":"30px auto","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="病虫图片" v-if="type!='cross' || (type=='cross' && !ro.bingchongtupian)" prop="bingchongtupian">
            <file-upload
            tip="点击上传病虫图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' class="upload" v-else label="病虫图片" prop="bingchongtupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.bingchongtupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}'  label="病虫害种类" prop="bingchonghaizhonglei">
            <el-select v-model="ruleForm.bingchonghaizhonglei" placeholder="请选择病虫害种类" :disabled=" false  ||ro.bingchonghaizhonglei" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="防治时间" prop="fangzhishijian">
              <el-date-picker
				  :disabled=" false  ||ro.fangzhishijian"
...
<script>
  export default {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='bingchongtupian'){
              this.ruleForm.bingchongtupian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`bingchonghaifangzhi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('bingchonghaifangzhi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`bingchonghaifangzhi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      bingchongtupianUploadChange(fileUrls) {
          this.ruleForm.bingchongtupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
